import { Graph, type CellStyle } from '@maxgraph/core';
...
function nodeStyle(node: DiagramNode): CellStyle {
	...
	const style: CellStyle = {
		whiteSpace: 'wrap',
		rounded: false,
		fillColor: node.style?.bg_color ?? 'var(--vscode-editorWidget-background)',
		fontColor: node.style?.text_color ?? 'var(--vscode-editor-foreground)',
		strokeColor: node.style?.border?.color ?? 'var(--vscode-editorWidget-border)',
		strokeWidth: borderWeight ?? 1,
	};

	if (node.style?.font?.family !== undefined) {
		style.fontFamily = node.style.font.family;
	}
	if (node.style?.font?.size !== undefined) {
